...
|Mediator|Mediator interface has methods to link and unlink a Participant instance, to notify all its Participant instance, and to notify a single one. The concrete mediator is ChatRoom class that implement the former interface and storage all the reference to its Participant instances in a HashMap. Participant interface has methods to get the own id and to operate to one or all other participants in the same mediator. Message is the model for what is sent and has a reference for the message itself, the sender and the date it was sent.|